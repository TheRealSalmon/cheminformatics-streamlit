...
def st_display_reaction(
    ...
    reactants_are_smarts=False, reagents_are_smarts=False, products_are_smarts=False,
    img_size=(400,100)
):
    def smi_to_smt(smi):
...
def st_run_reaction(smk, reactants, display_smk=True, img_size=(400,100)):
    if display_smk: st.markdown(f'SMIRKS: `{smk}`')
...
def st_check_smirks(reactantss, productss, answer, prompt, img_size=(400,100), idx_offset=0):
    st.write('')
    st.markdown(f"""
        ##### {prompt}
    """)
    ...
    for i, reactants_products in enumerate(zip(reactantss, productss)):
        ...
        if products == ['']:
            st_display_reaction(reactants=reactants, products='*', products_are_smarts=True)
            st.error('no reaction')
        else:
        ...
        if smk != '':
            ...
            if sorted(products) == sorted([Chem.MolToSmiles(output_product) for output_product in output_products]):
                st.success('Correct!')
        if i < n_schemes-1:
            st.markdown('---')
