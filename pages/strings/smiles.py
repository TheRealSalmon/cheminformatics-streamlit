...
def smiles_page():
    ...
    st.markdown(
        ...
        SMILES is a Simplified Molecular Input Line Entry System. In other
    ...
    st.markdown(
        """
        Atomic symbols have their first letter capitalized. However, the first
        letter may be lowercase if it is aromatic like this benzene.
    ...
    st.markdown(
        """
        Specifying atom-level properties requires the use of brackets.
    ...
    st.markdown(
        ...
        Bonds are far simpler than atoms as they come only in 4 categories:
        * single
        * double
        * triple
        * aromatic
    ...
    st_display_mol_with_smi(
        smiles, display_smiles=True, img_size=(200, 100), labels=labels
    )

    st.info(
        """SMILES don't respect "order" so `CCO` or `OCC` are equivalent."""
    )
    ...
    st.markdown(
        ...
        In a way, branching is SMILES inception, a SMILES string inside a SMILES
        string.
    ...
    st_display_mol_with_smi(
        smiles, display_smiles=True, img_size=(150, 100), labels=labels
    )
